...
const Motivational = () => {
  ...
  const fetchMotivationalQuotes = async () => {
    try {
      const data = await getQuotesByCategory('motivational', 5);
      setQuotes(data);
    } catch (err) {
      console.error('❌ Error fetching motivational quotes:', err);
      setError('Failed to load motivational quotes.');
    }
  };
  ...
  return (
    <section id="motivational" className="min-h-screen w-full px-4 py-12 flex flex-col items-center justify-start">
      ...
      <div className="flex justify-center mt-10">
        <button
          onClick={fetchMotivationalQuotes}
