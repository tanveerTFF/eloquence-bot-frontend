...
const Gamery = () => {
  ...
  const fetchGameryQuotes = async () => {
    try {
      const data = await getQuotesByCategory('gamery', 5);
      setQuotes(data);
    } catch (err) {
      console.error('❌ Error fetching gamery quotes:', err);
      setError('Failed to load gamery quotes.');
    }
  };
  ...
  return (
    <section id="gamery" className="min-h-screen w-full px-4 py-12 flex flex-col items-center justify-start">
