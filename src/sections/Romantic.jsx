...
const Romantic = () => {
  ...
  const fetchRomanticQuotes = async () => {
    try {
      const data = await getQuotesByCategory('romantic', 5);
      setQuotes(data);
    } catch (err) {
      console.error('❌ Error fetching romantic quotes:', err);
      setError('Failed to load romantic quotes.');
    }
  };
  ...
  return (
    <section id="romantic" className="min-h-screen w-full px-4 py-12 flex flex-col items-center justify-start">
