...
const Stoic = () => {
  ...
  const fetchStoicQuotes = async () => {
    try {
      const data = await getQuotesByCategory('stoic', 5);
      setQuotes(data);
    } catch (err) {
      console.error('❌ Error fetching stoic quotes:', err);
      setError('Failed to load stoic quotes.');
    }
  };
  ...
  return (
    <section id="stoic" className="min-h-screen w-full px-4 py-12 flex flex-col items-center justify-start">
